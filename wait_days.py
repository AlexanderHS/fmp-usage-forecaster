...
@time_limited_cache(max_age_seconds=CACHE_SECONDS)
def get_sorted_wait_dates(item_code: str = None, site_filter: str = None, customer_code: str = None) -> List[models.WaitDate]:
    raw_data = get_filtered_data(
        item_code=item_code, site_filter=site_filter, customer_code=customer_code)
    wait_dates = get_wait_dates(raw_data)
    wait_dates = sorted(wait_dates, key=lambda x: x.date)
    return wait_dates

@time_limited_cache(max_age_seconds=CACHE_SECONDS)
def get_wait_days_with_missing(item_code: str = None, site_filter: str = None, customer_code: str = None) -> List[models.WaitDate]:
    wait_dates = get_sorted_wait_dates(
        item_code, site_filter, customer_code)
...
@time_limited_cache(max_age_seconds=CACHE_SECONDS)
def get_smooth_wait_dates(item_code: str = None, site_filter: str = None, customer_code: str = None, smoothing: int = 3) -> List[models.WaitDate]:
    wait_days = get_wait_days_with_missing(
        item_code, site_filter, customer_code)
    wait_days = smooth_wait_dates(wait_days, smoothing)
    return wait_days
