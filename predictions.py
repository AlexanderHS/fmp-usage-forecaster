...
def get_orders(item_code: str, site_filter: str = None, site_filter2: str = None, dollars: bool = False) -> List[models.OrderDay]:
    ...
    if site_filter and site_filter2:
        filtered_data = [order for order in filtered_data if site_filter in order.site or site_filter2 in order.site]
...
def get_predictions_neural(item_code: str, days: int = 30, site_filter: str = None, site_filter2: str = None, dollars: bool = False) -> List[tuple]:
    ...
    predictions = list(
        zip(forecast['ds'].dt.strftime('%Y-%m-%d'), forecast['yhat1']))
